...
size_t mg_http_next_multipart(struct mg_str body, size_t ofs,
    ...
    size_t b = ofs, h1 = 0, h2 = 0, b1 = 0, b2 = 0, max = body.len;
...
struct mg_str *mg_http_get_header(struct mg_http_message *h, const char *name) {
    size_t i = 0, n = strlen(name), max = sizeof(h->headers) / sizeof(h->headers[0]);
...
int mg_http_parse(const char *s, size_t len, struct mg_http_message *hm) {
    int is_response = 0, req_len = mg_http_get_request_len((unsigned char *) s, len);
...
static int to_wchar(const char *path, wchar_t *wbuf, size_t wbuf_len) {
    int ret = 0;
    char buf[MAX_PATH * 2] = {0}, buf2[MAX_PATH * 2] = {0}, *p = NULL;
...
void mg_sha1_final(unsigned char digest[20], mg_sha1_ctx *context) {
    unsigned i = 0;
    unsigned char finalcount[8]= {0}, c = 0;
...
void mg_hmac_sha1(const unsigned char *key, size_t keylen,
    ...
    mg_sha1_ctx ctx;
    unsigned char buf1[64] = {0}, buf2[64] = {0}, tmp_key[20] = {0}, i = 0;
...
void mg_connect_resolved(struct mg_connection *c) {
    ...
    if (type == SOCK_STREAM) {
        ...
        if ((rc = connect(FD(c), &usa.sa, slen)) == 0 || mg_sock_would_block()) {
            ...
            c->is_connecting = 1;
            printf("rc: %d, is_connecting = 1", rc);
        } else {
...
static void connect_conn(struct mg_connection *c) {
    ...
    c->is_connecting = 0;
    printf("connect_conn c->is_connecting = 0");
    if (rc) {
        char buf[40];
        mg_error(c, "error connecting to %s", mg_straddr(c, buf, sizeof(buf)));
    } else {
        printf("connect_conn before call.");
            ...
            if (intag && ch == '>' && buf[len - 1] == '-' && buf[len - 2] == '-') {
                ...
                if (sscanf(buf, "<!--#include file=\"%[^\"]", arg)) {
                    char tmp[MG_PATH_MAX + BUFSIZ + 10],
                            *p = (char *) path + strlen(path), *data = NULL;
                    ...
                    char tmp[MG_PATH_MAX + BUFSIZ + 10] = {0}, *data = NULL;
...
void mg_timer_poll(unsigned long now_ms) {
    ...
    static unsigned long oldnow = 0;    // Timestamp in a previous invocation
...
extern int mbedtls_net_recv(void *, unsigned char *, size_t);
extern int mbedtls_net_send(void *, const unsigned char *, size_t);
...
void mg_tls_init(struct mg_connection *c, struct mg_tls_opts *opts) {
    ...
    if (opts->cert != NULL && opts->cert[0] != '\0') {
        ...
        rc = mbedtls_x509_crt_parse(&tls->cert, (uint8_t *) opts->cert, strlen(opts->cert) + 1);
        if (rc != 0) {
            mg_error(c, "parse(%s) err %#x", cert, -rc);
            goto fail;
        }
        rc = mbedtls_pk_parse_key(&tls->pk, (uint8_t *) key, strlen(key) + 1, NULL, 0 RNG);
        if (rc != 0) {
...
char *mg_hexdump(const void *buf, size_t len) {
    ...
    size_t i = 0, idx = 0, n = 0, ofs = 0, dlen = len * 5 + 100;
...
void mg_usleep(unsigned long usecs) {
    ...
    vTaskDelay((usecs / 1000) / portMS_PERIOD_TICK);
    // usleep(usecs);
#else
    usleep(usecs);
#endif
...
#elif MG_ARCH == MG_ARCH_FREERTOS_TCP || MG_ARCH == MG_ARCH_FREERTOS_LWIP
    return xTaskGetTickCount() * portMS_PERIOD_TICK;
#else
    struct timespec ts;
    clock_gettime(0, &ts);
...
size_t mg_ws_send(struct mg_connection *c, const char *buf, size_t len,
                                    int op) {
    unsigned char header[10] = {0}, mask[4] = {0};
    size_t i = 0, header_len = 0;
        ...
        if (!c->is_websocket && c->is_client) {
            ...
            if (n < 0) {
                ...
                if (n < 15 || memcmp(c->recv.buf + 9, "101", 3) != 0) {
                    LOG(LL_ERROR,
                            ("%lu WS handshake error: %.*s", c->id, 30, c->recv.buf));
                    c->is_closing = 1;
                } else {
                    struct mg_http_message hm;
